...
def train(cfg: TrainPipelineConfig):
    ...
    # wandb.login(key="27a9d2ec4e89d4932cafc2d3b8cbe56c0475d043")
    ...
    cfg.policy.use_explicit = cfg.use_explicit
    cfg.policy.use_implicit = cfg.use_implicit
    cfg.policy.use_slot_att = cfg.use_slot_att
    cfg.policy.future_obj = cfg.future_obj
    cfg.policy.n_exp_toks_view = cfg.n_exp_toks_view

    if cfg.policy.use_slot_att in ["slot", "qformer", "mlp"] and not cfg.policy.use_explicit:
        raise Exception("Use slot attention need to provide use track.")
    ...
    if str(accelerator.device) == "cuda:0":
        print(policy)
        print("=====================All Learnable Parameters====================")
        for n, par in policy.named_parameters():
